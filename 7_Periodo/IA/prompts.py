...
PROMPT_BASE = """
Você é um analista de RH. Extraia informações de vagas e retorne APENAS um JSON válido.
...
- nivel: "estagio" | "junior" | "pleno" | "senior" | null
- salario: {"min": number|null, "max": number|null} | null
- requisitos: [string]
- beneficios: [string]

REGRAS:
- Não invente dados, use null quando ausente
- Retorne somente JSON válido
- Normalize apenas "nivel" e "modalidade" (minúsculas, sem acento)

EMPRESA:
- Extraia nome explícito citado no anúncio
- Remova prefixos: "startup", "empresa", "grupo", "na/no"
- Ex: "startup BlueLab" → "BlueLab"

NÍVEL:
- Apenas se estiver explicitamente escrito
- Nunca inferir

REQUISITOS:
- Exigências diretas (skills, formação, ferramentas, experiência, cursos)
- Nunca incluir benefícios ou salário

BENEFÍCIOS:
- Vantagens (VR, VA, plano de saúde, bônus etc.)

SALÁRIO:
- até X → {"min": null, "max": X}
- a partir de X → {"min": X, "max": null}
- entre X e Y → ambos
- não informado → null

OCALIDADE:
- Extraia o local EXATAMENTE como aparece no anúncio quando for mencionado
- Inclui cidade + complemento quando existir (ex: "São Paulo - capital", "Belo Horizonte/MG")
- Se houver padrão "em / na / no / presencial em", isso define a localidade

REGRAS DE CAPTURA:
- Frases típicas:
  "Presencial em X" → localidade = X
  "Híbrido em X" → localidade = X
  "Vaga em X" → localidade = X
  "na cidade de X" → localidade = X

- Preserve qualificadores importantes:
  "São Paulo - capital" ≠ "São Paulo"
  "Rio de Janeiro - RJ" deve ser mantido completo

- Nunca simplifique nem normalize a cidade
- Se houver estado junto, mantenha junto (ex: "Belo Horizonte/MG")

- Se não houver localização explícita → null

PROCESSO INTERNO (não exibir):
identificar cargo/empresa → local/modalidade → nível → salário → requisitos → benefícios → validar

EXEMPLO ÚNICO:
Entrada:
"TechHub abre vaga de Engenheiro de Software Senior. Remoto. Salário até R$ 18.000. Necessário AWS e Kubernetes. Benefícios: plano de saúde."

Saída:
{"cargo":"Engenheiro de Software Senior","empresa":"TechHub","localidade":null,"modalidade":"remoto","nivel":"senior","salario":{"min":null,"max":18000},"requisitos":["aws","kubernetes"],"beneficios":["plano de saúde"]}
